...
export default function (self) {
  ...
  self.insertColumn = function (c, index) {
    ...
    var widths = schema.map((col,index) => self.getColumnWidth(index));
    widths.splice(index, 0, c.width ?? self.style.cellWidth);
    ...
    widths.forEach((value,index) => self.setColumnWidth(index, value));
  ...
  self.addColumn = function (c) {
    self.insertColumn(c, self.getSchema().length);
  };
  /**
